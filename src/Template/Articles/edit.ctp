    ...
    <nav class="navbar navbar-default" id="actions-sidebar">
        <ul class="nav nav-tabs">
            ...
            <li class="nav-item">
                <?=
                $this->Html->link(
                        $this->Html->tag('span ', '', array('class' => 'fa fa-plus-circle fa-lg')) . __('&nbsp;New Article'), array('action' => 'add'), array('escape' => false, 'class' => 'nav-link')
                )
                ?>
            </li>
            <li class="nav-item">
                <?=
                $this->Form->postLink(
                        $this->Html->tag('span', '', array('class' => 'fa fa-trash fa-lg')) . __('&nbsp;Delete'), array('action' => 'delete', $article->id), array(
                    'escape' => false,
                    'data-toggle' => 'tooltip',
            ...
            <li class="nav-item">
                <?=
                $this->Html->link(
                        $this->Html->tag('span', '', array('class' => 'fa fa-list fa-lg')) . __(' List Articles'), array('action' => 'index'), array('escape' => false, 'class' => 'nav-link')
                )
                ?>
            </li>
        </ul>
...
<?= $this->fetch('toolbar') ?>
        ...
        <div class="row-fluid clearfix">
            ...
            $this->Html->link($this->Html->image('article-images/' . $article->img, array(
                        ...
                        'width' => '110px')), 'img/article-images/' . $article->img, array(
